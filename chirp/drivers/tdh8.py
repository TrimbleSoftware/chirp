...
struct {
  ...
  u8 unused6:1,
     ...
     offset:2;
  u8 unused10:7,
     am_modulation:1; //     Per VFO AM modulation
} vfoa;
...
struct {
  ...
  u8 unused6:1,
     ...
     offsetb:2;
  u8 unused10:7,
     am_modulationb:1; //     Per VFO AM modulation
} vfob;
...
struct{
  ...
  u8 color;     // 0x1f32 radio meun bg color
} menu;
...
H8_GEN3_LIST = ["TD-H8 Gen 3", "TD-H8 Gen 3-HAM", "TD-H8 Gen 3-GMRS"]
...
ALL_MODEL = H8_LIST + H3_LIST + H3_PLUS_LIST + H8_GEN3_LIST + ["RT-730"]

TD_H8 = b"\x50\x56\x4F\x4A\x48\x1C\x14"
TD_H8_GEN3 = b"\x50\x56\x4F\x4A\x48\x3C\x14"
...
class TDH8(chirp_common.CloneModeRadio):
    ...
    def get_memory(self, number):
        ...
        try:
            ...
            LOG.error("Chan %d: Radio reported invalid power level %s (in %s)"
                      % (mem.number, _mem.lowpower, self._tx_power))
        ...
        if (chirp_common.in_range(mem.freq, self._rxbands) and
                not chirp_common.in_range(mem.freq, self.get_tx_bands())) or \
                _mem.am_modulation:
            mem.duplex = 'off'

        # narrow and wide
        if self.MODEL not in H3_PLUS_LIST and \
            chirp_common.in_range(mem.freq, self._airband) or \
                self.MODEL in H3_PLUS_LIST and _mem.am_modulation:  # AM
            if _mem.wide:  # narrow
                mem.mode = 'NAM'
            elif not _mem.wide:  # wide
                mem.mode = 'AM'
        else:  # FM
            if _mem.wide:  # narrow
                mem.mode = 'NFM'
    ...
    def set_memory(self, mem):
        ...
        if mem.mode in ['AM', 'NAM']:  # Per channel AM?
            _mem.am_modulation = 0b1  # AM
            mem.duplex = 'off'  # force duplex to be off as AM is RX only
        else:
    ...
    def _get_settings(self):
        ...
        if self.MODEL != "RT-730":
            ...
            if self.MODEL in H3_PLUS_LIST:
                rs = RadioSetting("ablock", "Channel No-A",
                                  RadioSettingValueInteger(
                                      1, 199,
                                      _settings.ablock))
                rs.set_doc("Default channel number for VFO A")
                rs[0].set_mutable(False)
                basic.append(rs)
            ...
            if self.MODEL in H3_PLUS_LIST:
                rs = RadioSetting("bblock", "Channel No-B",
                                  RadioSettingValueInteger(
                                      1, 199,
                                      _settings.bblock))
                rs.set_doc("Default channel number for VFO B")
                rs[0].set_mutable(False)
                basic.append(rs)
        ...
        if self.MODEL in H3_PLUS_LIST:
            rs = RadioSetting("menu.lang", "Language",
                              RadioSettingValueMap(
                                  self._lang_map,
                                  _menu.lang))
            rs.set_doc("Select radio menu language")
        basic.append(rs)

        if self.MODEL in H3_PLUS_LIST:
            rs = RadioSetting("menu.display", "Display Mode",
                              RadioSettingValueList(
                                  self._display_list,
                                  current_index=_menu.display))
            rs.set_doc("Select radio Display mode")
            basic.append(rs)

            rs = RadioSetting("menu.color", "Menu Color",
                              RadioSettingValueList(
                                  self._menucolor_list,
                                  current_index=_menu.color))
            rs.set_doc("Select radio menu text background color")
            basic.append(rs)
        ...
        if self.MODEL in H3_LIST + H3_PLUS_LIST:
            ...
            if self.MODEL not in H3_PLUS_LIST:
        ...
        if self.MODEL != "RT-730":
            ...
            if self.MODEL in H3_PLUS_LIST:
                rs = RadioSetting("am_modulation", "RX Modulation",
                                  RadioSettingValueList(
                                      self._rx_modulation_list,
                                      current_index=_vfoa.am_modulation))
                rs.set_doc("Select VFOA RX Modulation")
                abblock.append(rs)
            ...
            if self.MODEL in H3_PLUS_LIST:
                rs = RadioSetting("am_modulationb", "RX Modulation",
                                  RadioSettingValueList(
                                    self._rx_modulation_list,
                                    current_index=_vfob.am_modulationb))
                rs.set_doc("Select VFOB RX Modulation")
                abblock.append(rs)
        ...
        if self.MODEL != "RT-730":
            ...
            if self.MODEL in H8_LIST + H3_LIST:
                ...
                if self.MODEL in H8_LIST + H8_GEN3_LIST:  # only H8 has mid pwr
                    midpower = \
            ...
            if not isinstance(element, RadioSetting):
                ...
                try:
                    ...
                    if "." in name:
                        ...
                        _vfob.txtoneb = \
                            self._encode_vfo_tone(str(element.value))
                    elif name == "am_modulation":
                        _vfoa.am_modulation = int(element.value)
                    elif name == "am_modulationb":
                        _vfob.am_modulationb = int(element.value)
                    elif name == "astep":
                    ...
                    # Channel A
                        ...
                        val = int(str(element.value).replace(
                            '.', '').ljust(8, '0'))
                        if (val >= 1800000 and val <= 66000000):  # or \
                        ...
                        if (int(val) >= 0 and int(val) <= 5999750):
                            if int(val) == 0:
                                _aoffset.ofseta = [0x00, 0x00, 0x00, 0x00]
                    ...
                    # B channel
                        ...
                        if (int(val) >= 0 and int(val) <= 5999750):
                            if int(val) == 0:
                                _boffset.ofsetb = [0x00, 0x00, 0x00, 0x00]
...
class TDH3(TDH8):
    ...
    def get_features(self):
        ...
        rf.valid_modes = ["FM", "NFM", "AM", "NAM"]  # 25 kHz,12.5kHz, AM, NAM.
        rf.valid_tuning_steps = self._steps
        return rf
...
class TDH3_Plus(TDH3):
    ...
    _txbands = [(136000000, 174000000), (400000000, 4999995000)]
    ...
    def get_tx_bands(self):
        ...
        if _settings.tx220:
            bands.extend(self._tx220)
        if _settings.tx350:
            bands.extend(self._tx350)
        if _settings.tx500:
            bands.extend(self._tx500)
        bands.sort()
        return bands
...
@directory.register
class TDH8_GEN3(TDH3):
    MODEL = "TD-H8 Gen 3"
    H3_LIST += H8_GEN3_LIST
    _idents = [TD_H8_GEN3]
    _tx_power = [chirp_common.PowerLevel("Low",  watts=1.00),
                 chirp_common.PowerLevel("Mid",  watts=4.00),
                 chirp_common.PowerLevel("High", watts=8.00)]
...
@directory.register
@directory.detected_by(TDH8_GEN3)
class TDH8_GEN3_HAM(TDH3_HAM, TDH8_GEN3):
    MODEL = "TD-H8 Gen 3-HAM"


@directory.register
@directory.detected_by(TDH8_GEN3)
class TDH8_GEN3_GMRS(TDH3_GMRS, TDH8_GEN3):
    MODEL = "TD-H8 Gen 3-GMRS"
